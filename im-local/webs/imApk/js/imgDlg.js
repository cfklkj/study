    ...
    doneSubmit:function(res){  
        ...
        if (body.Code == 200){
           ...
           if (imgDlg.fileType.indexOf("image/") > -1) {
            chatLayout.send(actRecords.get(imDefine.act_selectContent),fileInfo, imDefine.chat_img) 
         }else{
            chatLayout.send(actRecords.get(imDefine.act_selectContent),fileInfo, imDefine.chat_file) 
         }
        }       
    },
